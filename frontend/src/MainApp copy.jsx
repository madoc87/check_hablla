...
import './App.css';
import { getAllServices, getCardDetails, updateCardOwner, sleep } from './utils/api';
...
export default function MainApp({ currentUser }) {
    ...
    const [selectedBoardForOwnerUpdate, setSelectedBoardForOwnerUpdate] = useState(Object.keys(BOARDS)[0]);
    ...
    const [cardOwnerUpdateResults, setCardOwnerUpdateResults] = useState({ summary: '', log: [], noCards: [], multipleCards: [] });
    ...
    // Função para atualizar o dono dos cartões, agora com a nova lógica de filtro
    const handleUpdateCardOwners = async () => {
        setCardOwnerUpdateIsLoading(true);
        setCardOwnerUpdateError('');
        setCardOwnerUpdateResults({ summary: '', log: [], noCards: [], multipleCards: [] });

        try {
            // 1. Busca todos os atendimentos do SETOR selecionado
            ...
            allServices.forEach(service => {
                if (!service.cards || service.cards.length === 0) {
                    noCards.push({ attendanceId: service.id, attendanceName: service.name, ownerId: service.user });
                } else if (service.cards.length > 1) {
                    multipleCards.push({ attendanceId: service.id, attendanceName: service.name, cardIds: service.cards, ownerId: service.user });
                } else {
                    attendanceData.push({ attendanceId: service.id, cardId: service.cards[0], ownerId: service.user });
                }
            });
            ...
            for (const data of attendanceData) {
                try {
                    const cardDetails = await getCardDetails(data.cardId);
                    // 2. FILTRA pelo quadro selecionado e verifica se não tem dono
                    if (!cardDetails.user && (cardDetails.board === selectedBoardForOwnerUpdate || cardDetails.board_id === selectedBoardForOwnerUpdate)) {
                        cardsToUpdate.push({ cardId: data.cardId, ownerId: data.ownerId });
                    }
                    await sleep(250); // Adiciona uma pausa de 250ms
                } catch (error) {
                    console.error(`Erro ao verificar detalhes do cartão ${data.cardId}:`, error);
                }
            }
            ...
            for (const card of cardsToUpdate) {
                try {
                    await updateCardOwner(card.cardId, card.ownerId);
                    log.push({
                        cardId: card.cardId,
                        status: 'success',
                        message: `Dono atualizado com sucesso para o usuário ${card.ownerId}`
                    });
                    updatedCount++;
                    await sleep(250); // Adiciona uma pausa de 250ms
                } catch (error) {
            ...
            setCardOwnerUpdateResults({
                summary: `Processo finalizado. ${updatedCount} de ${cardsToUpdate.length} cartões elegíveis foram atualizados.`,
                log: log,
                noCards: noCards,
                            ...
                            <div className="space-y-6">
                                <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg">
                                    ...
                                    <p className="text-blue-700 dark:text-blue-300 text-sm">
                                        Esta ferramenta verifica os cartões de um quadro selecionado que estão sem dono e atualiza
                                        automaticamente o dono do cartão para ser o mesmo dono do atendimento vinculado.
                                    </p>
                                </div>
                                ...
                                <div>
                                    <label htmlFor="boardSelectOwnerUpdate" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">2. Selecione o Quadro para Filtrar</label>
                                    <select id="boardSelectOwnerUpdate"
                                        value={selectedBoardForOwnerUpdate}
                                        onChange={(e) => setSelectedBoardForOwnerUpdate(e.target.value)}
                                        className="w-full p-2 bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg"
                                        disabled={cardOwnerUpdateIsLoading}
                                    >
                            ...
                            <div className="bg-gray-50 dark:bg-gray-900/50 p-6 rounded-lg">
                                ...
                                {cardOwnerUpdateResults.summary && (
                                    <div className="mb-4 p-3 bg-green-100 dark:bg-green-900/30 border border-green-200 dark:border-green-800 rounded-lg">
                                        <p className="text-green-700 dark:text-green-300 text-sm">{cardOwnerUpdateResults.summary}</p>
                                    </div>
                                )}
                                ...
                                <div className="h-96 overflow-y-auto space-y-4 pr-2">
                                    ...
                                    {!cardOwnerUpdateIsLoading && cardOwnerUpdateResults.log.length === 0 && !cardOwnerUpdateError && (
                                        <p className="text-gray-500 dark:text-gray-400 text-center pt-16">
                                            Selecione um quadro e clique no botão para iniciar o processo de atualização dos donos dos cartões.
                                        </p>
                                    )}
                                        ...
                                        <div>
                                            ...
                                            <div className="space-y-2">
                                                {cardOwnerUpdateResults.log.map((result, index) => (
                                                    <div
                                                        key={index}
                                                        className={`p-3 rounded-md text-sm ${result.status === 'success'
                                    ...
                                    {cardOwnerUpdateResults.noCards && cardOwnerUpdateResults.noCards.length > 0 && (
                                        <div className="mt-4">
                                            <h4 className="font-semibold text-yellow-700 dark:text-yellow-300 mb-2">Atendimentos sem cartão vinculado ({cardOwnerUpdateResults.noCards.length}):</h4>
                                            <div className="space-y-2">
                                                {cardOwnerUpdateResults.noCards.map((item, index) => (
                                                    <div key={index} className="p-3 bg-yellow-100 dark:bg-yellow-900/30 border border-yellow-200 dark:border-yellow-800 rounded-md text-sm">
                                                        ...
                                                        <p><strong>Dono:</strong> {item.ownerId}</p>
                                                    </div>
                                                ))}
                                            </div>
                                        </div>
                                    ...
                                    {cardOwnerUpdateResults.multipleCards && cardOwnerUpdateResults.multipleCards.length > 0 && (
                                        <div className="mt-4">
                                            <h4 className="font-semibold text-orange-700 dark:text-orange-300 mb-2">Atendimentos com mais de um cartão ({cardOwnerUpdateResults.multipleCards.length}):</h4>
                                            <div className="space-y-2">
                                                {cardOwnerUpdateResults.multipleCards.map((item, index) => (
                                                    <div key={index} className="p-3 bg-orange-100 dark:bg-orange-900/30 border border-orange-200 dark:border-orange-800 rounded-md text-sm">
                                                        <p><strong>Atendimento:</strong> {item.attendanceName} ({item.attendanceId})</p>
                                                        <p><strong>Cartões:</strong> {item.cardIds.join(', ')}</p>
                                                        <p><strong>Dono:</strong> {item.ownerId}</p>
                                                    </div>
                                                ))}
                                            </div>
